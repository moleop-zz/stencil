...
__device__ float GetElement(const Matrix A, int row, int col, int blockRow, int blockCol, dim3 grid, int num_gh, int size){
...
for (int i = 0; i < num_gh; ++i){
  ...
  if (blockRow == 0 && blockCol == 0 ){
  	if (row < BLOCKSIZE-1-num_gh && col < BLOCKSIZE-1-num_gh){
  		SetElement(submat, row, col, newValue);
  	}
  }else if (blockRow == 0 && blockCol != 0 && blockCol < grid.x-1){
   	if (col > num_gh && col < BLOCKSIZE-1-num_gh && row < BLOCKSIZE-1-num_gh)
   		SetElement(submat, row, col, newValue);
